...
function loadPDB(file){
  stage.loadFile(file, {defaultRepresentation: true}).then( function( comp){
      ...
      listeBlobs.push(file); //Ajout
      changeRepresentation(current_representation, current_scheme);
      addChoice("mySelect1");
      addChoice("mySelect2");
      console.log(file.getAsText("utf8")); //Ajout
  });
}
...
function dock(){    
	...
	let pdbFiles1, pdbFiles2;
	for (let i=0;i<listeBlobs.length;i++){
	    if (listeBlobs[i].name == prot1pdb){
		pdbFiles1=listeBlobs[i];
	    }
	    if (listeBlobs[i].name == prot2pdb){
		pdbFiles2=listeBlobs[i];
	    }
	}
	//
	var content = 0;
	/*fs.readFile(pdbFiles1, function read(err, data) {
	    if (err) {
		throw err;
	    }
	    content = data;
	...
	//	
	    ...
	    //alert(typeof pdbFiles1);
	    //alert(objectTest['nomprojet']);
	    alert("try to go to server");
            $.ajax({
                type: 'POST',
